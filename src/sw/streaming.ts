import { concat, HttpAgent, type QueryResponse } from '@dfinity/agent';
...
function isStreamingCallbackResponse(response: unknown): response is StreamingCallbackHttpResponse {
	return (
		typeof response === 'object' && response !== null && 'body' in response && 'token' in response
	);
}
