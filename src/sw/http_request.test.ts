...
it('should not set content-type: application/cbor and x-content-type-options: nosniff on non-ic calls to /api', async () => {
	...
	const response = await handleRequest(new Request('https://example.com/api/foo'));
...
it('should reject invalid certification (body hash mismatch)', async () => {
	...
	const response = await handleRequest(new Request(`https://${CANISTER_ID}.ic0.app/`));
	...
	expect(fetch.mock.calls[1][0]).toEqual(`https://ic0.app/api/v2/canister/${CANISTER_ID}/query`);
});
...
it('should accept valid certification without callbacks', async () => {
	...
	let [queryCall, req] = decodeContent<HttpRequest>(fetch.mock.calls[1], HttpRequestType);
...
it('should send canister calls to https://ic0.app for any mapped domain in hostnameCanisterIdMap', async () => {
	jest.setSystemTime(TEST_DATA.query_call_with_dscvr_canister_id.certificate_time);
	const queryHttpPayload = createHttpQueryResponsePayload(
...
it('should drop if-none-match request header', async () => {
	...
	mockFetchResponses(TEST_DATA.query_call.root_key, {
		...
		reqValidator: (req: HttpRequest) =>
			headerPresent(req, 'If-None-Match2') && !headerPresent(req, 'If-None-Match')
	});
...
it('should accept valid certification for index.html fallback', async () => {
	jest.setSystemTime(TEST_DATA.query_call_with_index_html_fallback.certificate_time);
	const queryHttpPayload = createHttpQueryResponsePayload(
...
it('should accept almost (but not yet) expired certificate', async () => {
	jest.setSystemTime(TEST_DATA.query_call.certificate_time + CERT_MAX_TIME_OFFSET);
	const queryHttpPayload = createHttpQueryResponsePayload(
		TEST_DATA.query_call.body,
		getResponseTypes(IDL.Text)[0],
		TEST_DATA.query_call.certificate
...
it('should reject expired certificate', async () => {
	jest.setSystemTime(TEST_DATA.query_call.certificate_time + CERT_MAX_TIME_OFFSET + 1);
	const queryHttpPayload = createHttpQueryResponsePayload(
		TEST_DATA.query_call.body,
		getResponseTypes(IDL.Text)[0],
		TEST_DATA.query_call.certificate
...
it('should reject certificate for different asset', async () => {
	...
	const response = await handleRequest(new Request(`https://${CANISTER_ID}.ic0.app/not-found`));
...
it('should accept valid certification using callbacks with primitive tokens', async () => {
	...
	const callbackPayload = createCallbackResponsePayload(' world!', CallbackResponseType);
	mockFetchResponses(
	...
	fetch.mock.calls
		.slice(1, 3)
		.forEach((call) =>
			expect(call[0]).toEqual(`https://ic0.app/api/v2/canister/${CANISTER_ID}/query`)
		);
	...
	let [callbackQuery, token] = decodeContent<string>(fetch.mock.calls[2], tokenType);
...
it('should accept valid certification using multiple callbacks with structured tokens', async () => {
	...
	const callbackPayload1 = createCallbackResponsePayload('lo wo', CallbackResponseType, [
		{
			counter: BigInt('32654653165435654'),
			variant: { test2: [true, false, true] }
		}
	]);
	const callbackPayload2 = createCallbackResponsePayload('rld!', CallbackResponseType);
	mockFetchResponses(
		TEST_DATA.query_call.root_key,
		{ query: queryHttpPayload },
...
it('should do update call on upgrade flag', async () => {
	...
	let [queryCall, queryReq] = decodeContent<HttpRequest>(fetch.mock.calls[1], HttpRequestType);
	expect(queryCall.method_name).toEqual('http_request');
	...
	// the update call
	expect(fetch.mock.calls[2][0]).toEqual(`https://ic0.app/api/v2/canister/${CANISTER_ID}/call`);
	let [updateCall, updateReq] = decodeContent<HttpRequest>(fetch.mock.calls[2], HttpRequestType);
...
function getResponseTypes(tokenType: IDL.Type): [IDL.Type, IDL.Type] {
	...
	const StreamingStrategy = IDL.Variant({
		Callback: IDL.Record({
			token: tokenType,
			callback: IDL.Func([tokenType], [StreamingCallbackHttpResponse], ['query'])
		})
	});
...
function fetchRootKeyResponse(rootKey: string) {
	return Agent.Cbor.encode({
		...
		impl_hash: '2e2e9c46e6139cd31c2aa736ae85b841af0a368c7083981def876cd4bcea9880',
...
function mockFetchResponses(
	...
	fetch.doMock(async (req) => {
		...
		if (httpPayloads[fetchCounter].reqValidator) {
			...
			let cborDecoded: { content: { arg: ArrayBuffer } } = Agent.Cbor.decode(body);
			let request = IDL.decode([HttpRequestType], cborDecoded.content.arg)[0];
			if (!httpPayloads[fetchCounter].reqValidator?.(request as unknown as HttpRequest)) {
...
function createUpgradeQueryResponse() {
	return createHttpQueryResponsePayload('', getResponseTypes(IDL.Text)[0], '', [], true);
}
...
function createCallbackResponsePayload(body: string, responseType: IDL.Type, token: any[] = []) {
	const response = {
		token: token,
		body: Array.from(new TextEncoder().encode(body))
	};
...
function headerPresent(req: HttpRequest, headerName: string) {
	let result = req.headers.find(
		([key, _]: [string, string]) => key.toLowerCase() === headerName.toLowerCase()
	);
