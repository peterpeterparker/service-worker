 ...
 *
 */
import { Actor, ActorSubclass, concat, HttpAgent } from '@dfinity/agent';
...
const legacyGateways = new Set(['boundary.dfinity.network', 'boundary.ic0.app']);
 ...
 */
async function removeLegacySubDomains(originalRequest: Request, gatewayUrl: URL): Promise<Request> {
...
export async function handleRequest(request: Request): Promise<Response> {
	...
	if (canisterResolver.isAPICall(request, currentGateway, lookup)) {
		const cleanedRequest = await removeLegacySubDomains(request, currentGateway);
	...
	if (lookup.canister) {
		try {
			...
			if (httpResponse.streaming_strategy.length !== 0) {
				buffer = concat(
					buffer,
					await streamContent(agent, lookup.canister.principal, httpResponse.streaming_strategy[0])
				);
			}
