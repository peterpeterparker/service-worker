...
describe('Canister resolver lookups', () => {
	...
	it('should resolve current gateway', async () => {
		global.self.location = mockLocation('https://rdmx6-jaaaa-aaaaa-aaadq-cai.ic1.app');
	...
	it('should resolve current gateway of known domain', async () => {
		...
		global.self.location = mockLocation(`${protocol}//www.knowncustomdomain.com`);
	...
	it('same domain lookup should use cache promises after first request', async () => {
		...
		for (let i = 0; i < numberOfCalls; ++i) {
			lookups.push(await resolver.lookup(new URL('https://www.customdappdomain.io')));
		}
	...
	it('should handle status codes that are not 2xx as web2 resources', async () => {
		...
		const web2resource = await resolver.lookup(new URL(`${self.location.protocol}//${hostname}`));
	...
	it('should fail lookup if gateway header is malformated', async () => {
		...
		const mockedHeaders = new Headers();
		mockedHeaders.set(domainLookupHeaders.canisterId, 'rdmx6-jaaaa-aaaaa-aaadq-cai');
	...
	it('should add gateway protocol as the current location protocol', async () => {
		const protocol = 'http:';
		global.self.location = mockLocation(`${protocol}//rdmx6-jaaaa-aaaaa-aaadq-cai.ic0.app`);
		...
		const customDomainLookup = await resolver.lookup(new URL('https://newdomain.com'));
		...
		expect((urlContainsCanisterLookup.canister as CanisterLookup).gateway.protocol).toEqual(
			protocol
		);
		expect((customDomainLookup.canister as CanisterLookup).gateway.protocol).toEqual(protocol);
	});
});
