...
export class CanisterResolver {
	...
	private static async storageConnection(): Promise<Storage<DomainsStorageDBSchema>> {
	 ...
	 */
	public isAPICall(request: Request, gateway: URL, lookup: DomainLookup): boolean {
	...
	public async isIcDomain(request: Request): Promise<boolean> {
		...
		if (lookup.canister) {
			...
			return (
				url.hostname.endsWith(gateway.hostname) && !url.hostname.endsWith(`raw.${gateway.hostname}`)
			);
		}
	...
	private async useCurrentGateway(lookup: DomainLookup): Promise<DomainLookup> {
		if (lookup.canister && lookup.canister.gateway.hostname !== self.location.hostname) {
	...
	private async fetchDomain(domain: URL, retries = 3): Promise<DomainLookup> {
		try {
			...
			if (successfulResponse && canisterId !== null && gateway !== null) {
				lookup.canister = {
			...
			if (err instanceof MalformedCanisterError || err instanceof MalformedHostnameError) {
				throw err;
			}
