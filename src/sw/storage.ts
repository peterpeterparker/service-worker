...
type StoreName<StorageDB> = StorageDB extends DBSchema ? KeysOf<StorageDB> : 'main';
...
interface StorageConnectOptions<StorageDB extends DBSchema | unknown = unknown> {
	name?: string;
	version?: number;
...
export class Storage<StorageDB extends DBSchema | unknown = unknown> {
	...
	static async connect<StorageDB extends DBSchema | unknown = unknown>({
		...
		const defaultStore = stores?.default ?? (idb.objectStoreNames[0] as typeof stores.default);
	...
	): Promise<PickDBSchemaValue<StorageDB, StoreName<StorageDB>>['body'] | undefined> {
		const store = opts?.storeName ?? this.defaultStore;
		const value: PickDBSchemaValue<StorageDB, StoreName<StorageDB>> = await this.idb.get(
			store,
			key
		);
	...
	async getAll(opts?: {
		...
		const values: PickDBSchemaValue<StorageDB, StoreName<StorageDB>>[] = await this.idb.getAll(
			store
		);
	 ...
	 */
	private async removeOutdatedRecords(opts?: { storeName?: StoreName<StorageDB> }): Promise<void> {
		const store = opts?.storeName ?? this.defaultStore;
